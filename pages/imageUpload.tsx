...
import FridgeMateFooter from "../mycomponents/FridgeMateFooter";
...
const processItemWithBackend = async (id: string, file: File) => {
  ...
  const chooseFromGallery = (id: string) => {
  ...
  // ---------- UI ----------
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#fffdfa] to-[#fff6ee] p-6 flex flex-col items-center text-gray-800 relative overflow-hidden">
          ...
          <div className="rounded-2xl bg-[#fff3e9] p-5 shadow-inner min-h-[220px] flex flex-col">
            {!selected && (
              <div className="flex-1 flex flex-col items-center justify-center text-center text-gray-500 gap-3">
                ...
                <div className="text-xs text-gray-400">Processing simulated — replace with real backend later.</div>
              </div>
            )}
            ...
            {selected && selected.status === "done" && selected.recipe && (
              <div className="flex-1 flex flex-col gap-4">
                ...
                <div className="mt-auto flex gap-3">
                  <button onClick={() => alert("Starting cooking mode...")} className="flex-1 py-2 bg-orange-500 text-white rounded-lg">Cook Now</button>
                  <button onClick={() => alert("Saved to favorites (demo).")} className="flex-1 py-2 border border-orange-200 text-orange-600 rounded-lg">Save</button>
                </div>

                <button onClick={() => { setSelectedId(null); }} className="mt-3 py-2 bg-white border border-orange-100 text-orange-600 rounded-lg">Done</button>
              </div>
            )}
  ...
  <div className="flex-1 flex flex-col items-center justify-center gap-2 text-red-600">
    ...
    <span className="text-sm">{selected.error}</span>
  </div>
)}
